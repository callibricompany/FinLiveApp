...
class PricerScreen extends React.Component {
  ...
  _displayBestResult () {
    return (
      <Text>Meilleurs résultats obtenus :</Text>
    );
  }
  ...
  _displayExpert () {
    return (
      <View>
      <Text>COUPON :</Text>
      <View style={{width:DEVICE_WIDTH*0.95/2, height: DEVICE_WIDTH*0.95/2}}>
        <Text> bonjour</Text>
      </View>
      </View>
    );
  ...
  _displayPricing = ({ item , index}) => {
    return (
          ...
          <FLPanel 
              ...
              marginTop={20}
              customLeftRight={()=>{
                index === 0 ? this.refs['carouselExpert'].snapToNext() : this.refs['carouselExpert'].snapToPrev();
              }}
              customRight={index === 0 ? true : false}
              customLeft={index === 0 ? false : true}
          >
  ...
  render() {
    ...
    return (
      <ScrollView style={[globalStyle.bgColor, {flex: 1}]}>
          <View style={{flex : 1, flexDirection : 'column', marginLeft: 0.025*DEVICE_WIDTH, marginRight: 0.025*DEVICE_WIDTH, borderWidth:0}}>

          <FLPanel 
                title={"NOMINAL INDICATIF"} 
                fontSizeTitle={18}
                updateActivatedDesactivated={this.changeStateActivatedDesactivated.bind(this)}
                idComponent={"UF"} 
                hauteur={100} 
            ...
            <View style={{flexDirection: 'row'}}>
              <View style={{flex: 0.6, justifyContent: 'center', borderBottomColor: 'gray', borderBottomWidth: 1, height:35, paddingRight:5, paddingLeft: 5}}>
                    <TextInput
                      style={{height: 33}}
                      onChangeText={text => {
                        var product = this.state.product;
            ...
            >
                <FLSlider
                    ...
                    spreadScale={1}
                    //activated={!this.state.product["UF"].isActivated}
                    sliderLength={DEVICE_WIDTH*0.85}
                    callback={(value) => {
                      var product = this.state.product;
            ...
            <View style={{flex : 1, flexDirection : 'column',borderColor:'red', borderWidth:0}}>
